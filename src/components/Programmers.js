...
export default function Programmers() {
  ...
  const [programmersList, programSetter] = useState(listOfAwesome);
  const [featuredProgrammerId, featuredProgramIdSetter] = useState(null);
  const getNameOfFeatured = () => {
    ...
    const programmerLocation = featuredProgrammerId - 1;

    const programmerName = programmersList[programmerLocation].name;
    return programmerName;
    // const luckyDev = programmersList.filter()
  };

  console.log(programmersList);
  console.log(featuredProgrammerId);
  //   console.log()
  // console.log()
  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: featuredProgrammerId ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className="programmers">
        {
          ...
          programmersList.map((dev) => (
            <div className="programmer" key={dev.id}>
              ...
              <button
                onClick={() => {
                  featuredProgramIdSetter(dev.id);
                }}
              >
                Feature
              </button>
